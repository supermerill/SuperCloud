...
namespace supercloud::test {
  ...
  //      std::filesystem::path net_prop_file = tmp_dir_path / "clusterIds.properties";
        ...
        //launch first peer
		std::shared_ptr<PhysicalServer> net{ new PhysicalServer{tmp_dir_path} };
	...
	SCENARIO("ByteBuff Trail UInt64") {
		ByteBuff buff;
		GIVEN(("0")) {
			buff.reset().putTrailULong(0).flip();
			THEN("check size and value") {
				REQUIRE(buff.limit() == 1);
				REQUIRE(buff.getTrailULong() == 0);
			}
		}
		GIVEN(("1")) {
			buff.reset().putTrailULong(1).flip();
			THEN("check size and value") {
				REQUIRE(buff.limit() == 1);
				REQUIRE(buff.getTrailULong() == 1);
			}
		}
		GIVEN(("TrailULong 4bits")) {
			...
			THEN("check start at " + std::to_string(val)) {
				buff.putTrailULong(val).flip();
				REQUIRE(buff.limit() == nb_bits);
				REQUIRE(buff.getTrailULong() == val);
			}
			val = (1 << (7 * nb_bits - 1)) - 1;
			THEN("check positive end at " + std::to_string(val)) {
				buff.putTrailULong(val).flip();
		...
		GIVEN(("-1 = " + std::to_string(uint64_t(-1)))) {
			buff.reset().putTrailULong(uint64_t(-1)).flip();
			THEN("check size and value") {
				REQUIRE(buff.limit() == 9);
				REQUIRE(buff.getTrailULong() == uint64_t(-1));
			}
		}
		GIVEN(("max = " + std::to_string(std::numeric_limits<uint64_t>::max()))) {
			buff.reset().putTrailULong(std::numeric_limits<uint64_t>::max()).flip();
			THEN("check size and value") {
				REQUIRE(buff.limit() == 9);
				REQUIRE(buff.getTrailULong() == std::numeric_limits<uint64_t>::max());
			}
		}
	}
